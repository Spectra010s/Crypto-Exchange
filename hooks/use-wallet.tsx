...
import {
  useAccount,
  useBalance,
  useDisconnect as useEthDisconnect,
} from "wagmi";
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  // Fallback token prices for development
  const fallbackTokenPrices = {
    ethereum: 3200,
    solana: 98,
    "matic-network": 0.85,
    arbitrum: 1.2,
  ...
  // Fallback historical buy prices for gain/loss calculation
  const fallbackBuyPrices = {
    ethereum: 2800,
    solana: 85,
    "matic-network": 0.75,
    arbitrum: 1.0,
  ...
  const updateWalletData = async () => {
    if (ethConnected && ethAddress) {
      ...
      if (ethBalance) {
        ...
        const currentPrice = fallbackTokenPrices[tokenId] || 3200;
        ...
        balances.push({
          ...
          address: ethAddress,
          currentPrice,
          priceChange24h: currentPrice * 0.02, // 2% fallback change
      ...
      try {
        ...
        const currentPrice = fallbackTokenPrices[tokenId] || 98;
        ...
        try {
          ...
          if (realPrices["solana"]) {
            ...
            balances[0].gainLoss =
              (currentPrice - buyPrice) * balances[0].balance;
            balances[0].gainLossPercentage =
              ((currentPrice - buyPrice) / buyPrice) * 100;
          }
        } catch (error) {
